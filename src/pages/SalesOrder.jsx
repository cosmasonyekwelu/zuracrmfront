...
export default function SalesOrders() {
  ...
  const load = async () => { const r = await API.get("/salesorders", { params: { search: q } }); setRows(r.data || []); };
  ...
  const submit = async (e) => {
    ...
    if (editing) {
      const { _id, ...p } = payload;
      await API.patch(`/salesorders/${editing}`, p);
      setRows(rs => rs.map(x => x._id === editing ? { ...x, ...p } : x));
    } else {
      const r = await API.post("/salesorders", payload);
        ...
        <div className="container" style={{ padding: "8px 0" }}>
          ...
          <div className="card shadow-lg" style={{ marginBottom: 14 }}>
            <form onSubmit={submit} style={{ display: "grid", gap: 12 }}>
              <div className="row">
                <input className="input" placeholder="Order # (auto if empty)" value={form.number} onChange={e => setForm(f => ({ ...f, number: e.target.value }))} />
              ...
              <div className="row" style={{ justifyContent: "flex-end", gap: 14 }}>
                ...
                <div className="row" style={{ gap: 10 }}>
                  <button className="btn btn-primary">{editing ? "Save order" : "Create order"}</button>
                  {editing && (
                    <button
                      type="button"
          ...
          <div className="card shadow-lg">
            <div className="row" style={{ alignItems: "center", marginBottom: 10 }}>
              <input className="input" placeholder="Search orders…" value={q} onChange={e => setQ(e.target.value)} />
            </div>
            <table className="table">
              ...
              <tbody>
                {filtered.map(o => (
                  <tr key={o._id}>
                    <td><strong>{o.number || o._id.slice(-6).toUpperCase()}</strong></td>
                    <td>{o.account}</td>
                    <td>{o.date ? new Date(o.date).toLocaleDateString() : "—"}</td>
                    <td><span className={`chip ${o.status === "Fulfilled" ? "chip--emerald" : o.status === "Cancelled" ? "chip--rose" : "chip--amber"}`}>{o.status}</span></td>
                    <td>{fmtNGN(o.total)}</td>
                    <td>
                      <div className="row" style={{ gap: 8 }}>
                        ...
                        <button className="btn btn-ghost" onClick={() => remove(o._id)}>Delete</button>
                      </div>
                    </td>
                  </tr>
                ))}
                {!filtered.length && <tr><td colSpan={6} style={{ textAlign: "center", padding: 20, color: "#64748b" }}>No orders</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
